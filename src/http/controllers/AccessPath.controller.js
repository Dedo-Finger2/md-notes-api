...
export default class AccessPathController {
    accessPathUseCase

    constructor({ accessPathUseCase }) {
        this.accessPathUseCase = accessPathUseCase
    }
    ...
    async handle(request, response) {
        try {
            ...
            let { path } = v.parse(requestHeadersSchema, request.params)
            if (path === ':path') {
                path = userCredentials.defaultPath
            }
            const output = await this.accessPathUseCase.execute({
                path,
